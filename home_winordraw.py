...
def select_features(X_train, y_train, X_test):
    ...
    grid_scorer = metrics.make_scorer(best_mcc_score, greater_is_better=True)
    fs = SelectFromModel(XGBClassifier(random_state=0))
...
# df = df[df['odds_team_a'] >= 1.702].copy()
print(len(df))
...
df = df[df['odds_team_a'] >= 1.618].copy()
# df['odds_team_a'] = df.odds_team_a
# df['odds_draw'] = 1/df.odds_draw
# df['odds_team_b'] = 1/df.odds_team_b
...
# df['draw'] = np.where((df['team_a_ft_result'] == df['team_b_ft_result']) & (df['odds_draw'] <= 4), 1, 0)
...
knn = VotingClassifier(estimators=[('Logistic', lr),('Catboost', nb)], voting='soft', weights=[1.5,1])
knn.fit(X_train, y_train)
...
# Voting Classifier is the best for home win or draw
# Threshold: 0.65
