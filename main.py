                    ...
                    if team_b_num_matches and team_a_num_matches == 5:
                        ...
                        try:
                            ...
                            if len(match_records) == 1:
                                try:
                                    existing_df = pd.read_csv(fooball_data)
                                    if existing_df.empty:
                                        df.to_csv(fooball_data, index=False)  # used to save the last x number of matches into a csv
                                    else:
                                        df.to_csv(fooball_data, mode='a', index=False, header=False)
                                except:
                                    df.to_csv(fooball_data, index=False)  # used to save the last x number of matches into a csv
